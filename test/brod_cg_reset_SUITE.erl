...
t_set_then_reset(Config) when is_list(Config) ->
  %% Topic/Partitions to fetch from commit history for validation
  Topics = [ {<<"brod-group-subscriber-1">>, [0, 1, 2]}
           , {<<"brod-group-subscriber-2">>, [0, 1, 2]}
           ],
  Offsets0 =
    [{<<"brod-group-subscriber-1">>,
      [{0, 0}, {1, 0}, {2, 0}]},
     {<<"brod-group-subscriber-2">>,
      [{0, 1}, {1, 1}, {2, 1}]}
    ],
  ok = do_reset(Offsets0),
  {ok, Rsp0} =
    brod_utils:fetch_committed_offsets(?BROKERS, [], ?GROUP_ID, Topics),
  ok = assert_offsets(Offsets0, Rsp0),
  Offsets1 =
    [{<<"brod-group-subscriber-1">>,
      [{0, 1}, {1, 1}, {2, 1}]},
     {<<"brod-group-subscriber-2">>,
  ...
  ok = do_reset(Offsets1),
  {ok, Rsp1} =
    brod_utils:fetch_committed_offsets(?BROKERS, [], ?GROUP_ID, Topics),
  ok = assert_offsets(Offsets1, Rsp1),
  ok.
...
do_reset(Offsets) ->
  {ok, Pid} = brod_cg_reset:start_link(?CLIENT_ID, ?GROUP_ID, -1, Offsets),
  ok = brod_cg_reset:sync(Pid),
  ok = brod_cg_reset:stop(Pid),
  ok.
